...
type Prediction = { frequency_hz: number[]; zcm_ohm: number[]; zdm_ohm: number[]; version: string; quota?: Quota; validation?: Validation; surface_1mhz?: Surface };
...
export default function CmcDemo() {
  ...
  const [quota,setQuota]=useState<Quota|null>(null),[showAccess,setShowAccess]=useState(false),[email,setEmail]=useState(""),[code,setCode]=useState(""),[codeSent,setCodeSent]=useState(false),[accessBusy,setAccessBusy]=useState(false),[accessMessage,setAccessMessage]=useState("");
  const familyLabels=useMemo(()=>families.map(f=>f.join(" / ")),[]);
  useEffect(()=>{fetch("/api/cmc/quota",{cache:"no-store"}).then(async r=>{const x=await r.json();if(r.ok)setQuota(x)}).catch(()=>undefined)},[]);
  async function requestCode(){setAccessBusy(true);setAccessMessage("");try{const r=await fetch("/api/cmc/quota/request-code",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email})}),x=await r.json();if(!r.ok)throw new Error(x.error);setCodeSent(true);setAccessMessage(x.message)}catch(e){setAccessMessage(e instanceof Error?e.message:"인증번호 발송에 실패했습니다.")}finally{setAccessBusy(false)}}
  async function verifyCode(){setAccessBusy(true);setAccessMessage("");try{const r=await fetch("/api/cmc/quota/verify-code",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,code})}),x=await r.json();if(!r.ok)throw new Error(x.error);setQuota(x);setShowAccess(false)}catch(e){setAccessMessage(e instanceof Error?e.message:"이메일 인증에 실패했습니다.")}finally{setAccessBusy(false)}}
  async function predict(){setBusy(true);setMessage("특성을 계산하고 있습니다.");setData(null);try{const r=await fetch("/api/cmc/predict",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(design)}),x=await r.json();if(x.quota)setQuota(x.quota);if(r.status===429&&!x.quota?.bonus)setShowAccess(true);if(!r.ok)throw new Error(x.error);setData(x);setMessage(`예측 완료 · ${x.version}`)}catch(e){setMessage(e instanceof Error?e.message:"예측 실패")}finally{setBusy(false)}}
  return <main className="min-h-screen bg-slate-100 px-5 py-12 text-slate-800"><div className="mx-auto max-w-6xl space-y-6"><Link href="/solution" className="text-sm text-blue-700">← Solutions</Link><header className="rounded-2xl bg-[#14283f] p-8 text-white"><p className="text-sm tracking-widest text-blue-200">EMxAI · ENGINEERING DEMO</p><h1 className="mt-3 text-3xl font-bold">CE-CMF-Filter-PCB Sim</h1><p className="mt-3 text-slate-200">CMC 형상 · 특성 예측 · 검증 결과 · 대표 SPICE</p><p className="mt-5 text-sm text-blue-200">80-case-2026-09-09 / GPR + PCA8 / 150 kHz–30 MHz</p></header>
    ...
    {tab==="prediction"?<section className="rounded-2xl bg-white p-7 shadow-sm"><div className="grid gap-4 sm:grid-cols-4"><label>권선 수<input className="mt-2 w-full rounded border p-2" type="number" min="6" max="12" value={design.turns} onChange={e=>setDesign({...design,turns:+e.target.value})}/></label><label>코어 ID / OD / H (mm)<select className="mt-2 w-full rounded border p-2" value={design.family} onChange={e=>setDesign({...design,family:+e.target.value})}>{familyLabels.map((f,i)=><option value={i} key={f}>{f}</option>)}</select></label><label>선경 (mm)<input className="mt-2 w-full rounded border p-2" type="number" min=".5" max="1.1" step=".1" value={design.wire} onChange={e=>setDesign({...design,wire:+e.target.value})}/></label><label>피치 (°)<input className="mt-2 w-full rounded border p-2" type="number" min="2.1" max="5" step=".1" value={design.pitch} onChange={e=>setDesign({...design,pitch:+e.target.value})}/></label></div>
      <div className="mt-6 flex flex-wrap items-center gap-4"><button disabled={busy||quota?.remaining===0} onClick={predict} className="rounded-lg bg-blue-700 px-7 py-3 font-semibold text-white disabled:opacity-50">{busy?"계산 중…":"특성 예측"}</button>{quota&&<p className="text-sm font-medium">오늘 {quota.used}/{quota.limit}회 사용 · {quota.remaining}회 남음</p>}{quota&&!quota.bonus&&quota.remaining===0&&<button onClick={()=>setShowAccess(true)} className="rounded-lg border border-blue-600 px-5 py-3 font-semibold text-blue-700">이메일 인증으로 100회 추가</button>}</div><p role="status" className="mt-4 text-sm">{message}</p>
    ...
    {showAccess&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" role="dialog" aria-modal="true"><div className="w-full max-w-md rounded-2xl bg-white p-6"><div className="flex justify-between"><h2 className="text-xl font-bold">오늘 100회 추가</h2><button onClick={()=>setShowAccess(false)} aria-label="닫기">×</button></div><p className="mt-2 text-sm">이메일로 받은 인증번호를 입력하면 오늘 총 200회까지 사용할 수 있습니다.</p><label className="mt-5 block text-sm font-semibold">이메일<input type="email" value={email} onChange={e=>setEmail(e.target.value)} disabled={codeSent} className="mt-2 w-full rounded-lg border p-3"/></label>{!codeSent?<button onClick={requestCode} disabled={accessBusy||!email} className="mt-4 w-full rounded-lg bg-blue-700 p-3 font-semibold text-white disabled:opacity-50">인증번호 받기</button>:<><label className="mt-4 block text-sm font-semibold">인증번호<input inputMode="numeric" maxLength={6} value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,""))} className="mt-2 w-full rounded-lg border p-3"/></label><button onClick={verifyCode} disabled={accessBusy||code.length!==6} className="mt-4 w-full rounded-lg bg-blue-700 p-3 font-semibold text-white disabled:opacity-50">인증하고 100회 추가</button></>}{accessMessage&&<p className="mt-3 text-sm">{accessMessage}</p>}</div></div>}
  </main>;
}
