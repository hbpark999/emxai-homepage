...
const out=path.resolve(__dirname,'../../../deliverables/z0-mcp-slides/CMC-Coil-MCP-training.pptx');
...
// 41. Actual GUI anatomy
{
 ...
 const outs=[['형상','3D 이중 권선 미리보기'],['특성','|ZCM| · |ZDM| 주파수 곡선'],['불확도','검증 P90 오차 범위'],['모델','SPICE Subcircuit 생성']];
 outs.forEach((v,i)=>{const y=4.72+i*.35;text(s,v[0],9.30,y,.70,.20,{fontSize:10.5,color:C.gray});text(s,v[1],10.12,y,2.48,.20,{fontSize:10.8,bold:true,color:i===3?C.blue:C.navy});});
 footer(s,'GUI 입력값이 Coil 형상, 임피던스 곡선, SPICE 모델을 함께 결정합니다.',41);
}
